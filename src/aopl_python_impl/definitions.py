...
import string
import cmath
# FIX: Add 'Tuple' to the import list
...
ValueTuple = tuple[complex, int]

IMAGINARY_UNIT_J: ValueTuple = (complex(0, 1), 0)

LETTER_TO_EXPONENT_MAP: Dict[str, int] = {letter: i + 1 for i, letter in enumerate(string.ascii_lowercase)}
EXPONENT_TO_LETTER_MAP: Dict[int, str] = {i + 1: letter for i, letter in enumerate(string.ascii_lowercase)}

# --- CORRECTED TOKEN SPECIFICATION ---
_opt_sign = r"[+-]?"
_number_bare = r"(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?"
_number_signed = fr"{_opt_sign}{_number_bare}"
_word_simple = r"[a-zA-Z]+"
...
TOKEN_SPECIFICATION: List[Tuple[str, str]] = [
    ...
    # Make sure '**' is checked for before single '*'
    ('OPERATOR', r"\*\*|[\+\-\*\/\^]"),
...
OPERATORS: Dict[str, Dict] = {
    ...
    '/': {'precedence': 3, 'associativity': 'left'},
    # '^' is left-associative for chained calculations
    '^': {'precedence': 5, 'associativity': 'left'},
    # Add '**' for right-associative mathematical convention
